...
- **Persistence:** Supabase PostgreSQL (`@supabase/supabase-js`) — Server Actions 경유
- **LLM:** Google Gemini (`@google/genai`, 모델 `gemini-2.5-flash-lite`) — Next.js API Route 경유
...
프로젝트 루트에 `.env.local` 파일을 생성하고 키를 설정한다 (`.env.example` 참고).
...
2. DB 접근(CRUD)은 반드시 Server Actions(`src/app/actions/`)를 통해서만 수행할 것
3. 메모 ID 생성은 uuid v4만 사용 (Server Action 내에서 생성 후 DB에 전달)
...
│   ├── api/summarize/ # LLM 요약 API Route
│   └── page.tsx       # 메인 페이지
...
└── utils/
    ...
    │   └── mapper.ts  # DB row ↔ 앱 타입 변환
    └── seedData.ts    # 개발용 샘플 데이터 참조
```
...
- **[유틸리티 (Utils)](./src/utils/AGENTS.md)** — Supabase 클라이언트, 매퍼, 헬퍼 함수 작업 시
- **Server Actions (`src/app/actions/memoActions.ts`)** — Supabase DB CRUD 작업 시
