...
└── api/
    └── summarize/
        └── route.ts    # POST /api/summarize (Gemini 요약)
```

### API Route 컨벤션

- `app/api/<name>/route.ts` 패턴으로 작성, `POST` 등 HTTP 메서드 함수를 export.
- 외부 SDK(예: `@google/genai`) 호출이나 환경 변수(`process.env.*`) 접근은 반드시 API Route에서 처리하여 브라우저에 시크릿이 노출되지 않게 한다.
- Node 전용 SDK가 필요한 경우 `export const runtime = 'nodejs'` 명시.
- 응답은 `NextResponse.json({...}, { status })` 사용, 에러 메시지는 한국어로 제공.
